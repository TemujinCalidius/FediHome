    ...
    >
      ...
      >
        ...
        <img
          ...
          className={`max-w-[90vw] max-h-[80vh] object-contain select-none transition-transform duration-250 ${visible ? "scale-100" : "scale-95"}`}
          draggable={false}
        />
        {images[index].alt && (
        ...
        {(images[index].buyHref || images[index].detailHref) && (
          <div className="mt-4 flex items-center gap-4 flex-wrap justify-center">
            {images[index].buyHref && (
              <a
                href={images[index].buyHref}
                onClick={(e) => e.stopPropagation()}
                className="bg-accent-500 hover:bg-accent-400 text-white text-sm font-semibold px-5 py-2.5 rounded-lg shadow-lg transition-colors flex items-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                </svg>
                Buy this print{images[index].buyPrice ? ` — ${images[index].buyPrice}` : ""}
              </a>
            )}
            {images[index].detailHref && (
              <a
                href={images[index].detailHref}
                onClick={(e) => e.stopPropagation()}
                className="text-white/60 hover:text-white text-xs transition-colors underline-offset-4 hover:underline"
              >
                View post
              </a>
            )}
