...
- **Prisma** for all database access. The schema is in `prisma/schema.prisma`. FediHome doesn't track migration files — after changing the schema, run `npx prisma db push` to sync your local database, and document the change in the changelog under a "Schema" heading so operators know to run `db push` after upgrading.
  ...
  - **Never put a bare data backfill in one.** These files re-run, and a backfill's `WHERE` clause is almost never a description of history — it is usually a description of the current steady state. Fence it behind a creation-time guard, or let a column `DEFAULT` do it. `src/lib/__tests__/manual-migrations.test.ts` enforces this; the reasoning is in [`prisma/manual-migrations/README.md`](prisma/manual-migrations/README.md) and the incident is #384.
...
**Changelog (required).** Every pull request must add an entry to [`CHANGELOG.md`](CHANGELOG.md) under the `## Unreleased` heading (create it if it's missing), grouped under `### Added` / `### Changed` / `### Fixed` / `### Security`. CI enforces this on pull requests. If a change genuinely warrants no entry (e.g. a documentation-only PR, a CI-config tweak, or a typo fix), apply the `skip-changelog` label to bypass the check. At release time, `## Unreleased` is renamed to the new version. CI also checks **structure**: released sections are immutable — after stripping `## Unreleased`, a PR's changelog must match `main`'s released history exactly, so entries can only be added under `## Unreleased` and released headings can't be edited or dropped by a bad merge. (A deliberate fix to released history takes the `changelog-resync` label.) Releases themselves follow [`docs/releasing.md`](docs/releasing.md) — `scripts/prepare-release.mjs` does the rename + version bump.
