...
### Security
- **Another server could no longer point your instance at things on your own network.** When FediHome fetches an account or delivers a post, it checks that the address is a real public one before connecting — but it only checked the *first* address. A server could answer "it moved over there" and send the request somewhere private instead: a database on the same machine, or the metadata service your hosting provider runs. Every hop is now checked, and delivery refuses to follow a redirect to a different server at all, since nothing legitimate needs that. Found during an internal audit; no evidence of it being used.
