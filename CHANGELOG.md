...
- The storage-measurement tests no longer depend on whether the machine running them happens to have media in it — they were measuring the developer's own uploads alongside their fixtures, so they only passed on an empty checkout and failed on any real install. Fixing that revealed the measurement's headline behaviour, counting media left behind after moving the uploads directory, had never actually been tested; it is now. A related test could also leave a stray file behind if it was interrupted, which permanently broke the storage tests on that machine afterwards.
- The test suite no longer reads the machine's own configuration. Eleven tests quietly assumed that the settings they were checking for were unset, so they passed on a maintainer's laptop and failed on a machine actually configured to run FediHome — the same class of problem as the storage tests above, in its other form. Settings are now cleared for every test file, so "unset" means unset everywhere. Fixing it turned up a second gap: six checks covering the app-authorisation screen had never once run the path a reverse-proxied install takes, because the stand-in request they used was missing a field only that path reads. That path is covered now, including the per-visitor budget that stops one visitor spending another's.

### Security
- **Blocking someone now stops every kind of Bluesky message reaching them** (#577) — blocking already stopped FediHome liking or reposting a blocked account's posts, but four other things still went out: **direct messages, replies, follows, and replies crossposted from your own site**. Each notifies the person you blocked. The crossposted reply was the worst of them, because the retry queue could deliver one *after* you blocked them — you block someone, and a reply you wrote earlier arrives anyway. All four are refused now, before any connection is made, so nothing reaches their server at all. Group conversations are checked against every member rather than just whoever wrote last. Incoming messages were also matching domain blocks against the sender's internal identifier instead of their handle, which meant blocking a whole domain quietly failed to cover some of its accounts; that now abstains instead of guessing. Blocking a single account always worked throughout — it was the domain half, and the outbound direction, that were missing.
...
### Changed
- Dependency refresh: `next` + `eslint-config-next` + `@next/eslint-plugin-next` 16.3.1, `@atproto/api` 0.20.40, `pg` 8.23.0, `nodemailer` 9.0.5, `marked` 18.0.9, `sanitize-html` 2.17.7, `postcss` 8.5.26, `@types/node` 26.2.0, `@types/pg` 8.21.0, `eslint` 10.8.1, `tsx` 4.23.12. (`typescript` remains held at 6.x and `@typescript-eslint` at 8.61.1 — see #234.)

### Fixed
- **Blocked accounts stay hidden in threads and reply lists** (#559) — blocking already kept someone out of your timeline, but four places still let them through. Opening one of their posts directly showed it; a thread showed them if they appeared further up the conversation; your own replies list showed their name, picture and a snippet of what they wrote whenever you'd replied to them; and the Explore tab could still credit them as the reason a post was surfaced. Worse than any of that, asking for like and boost counts on one of their posts **contacted their server** — the exact thing blocking is supposed to stop. All four are fixed, and the check that was meant to catch them has been rebuilt: it only looked at one kind of database read, and it trusted a file-level note that was true of one query and false of another right beside it.
- **Ordinary installs stop building 100MB they never use** (#557) — FediHome always produced a second, self-contained copy of the server for the Docker image, even on installs that don't use Docker. It was never run, and it made `next start` warn on every restart that you should run it instead — advice that would have given you a site with **no styling at all**, because the stylesheets aren't copied into it. That copy is now only built when a container image is being made. A normal `npm run build` is about **110MB smaller**, the restart warning is gone, and the misleading advice with it. Nothing changes for Docker. If you deliberately start FediHome with `node .next/standalone/server.js`, switch to `npm start` — or set `FEDIHOME_STANDALONE=1` before building.
- **The support bundle stops contradicting itself about rate limiting** (#551) — the bundle has a line saying whether FediHome can tell your visitors apart, and it worked that out from your settings rather than from what the site was actually doing. Those agree until you mistype the header name — at which point FediHome correctly ignores it and falls back to treating everyone as one visitor, while the bundle confidently reported the opposite. That's the one line you'd check to find out why your limits were behaving oddly, so a wrong answer there sent you looking in the wrong place. It now asks the same code that does the keying, and when it says SHARED it also says why — whether the setting is off, or set to something unrecognised. (v1.27.1's notes said that line reported which of the two your site was using; it reported which one your settings implied.)
